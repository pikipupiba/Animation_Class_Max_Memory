...
private:
	...
	// Each animation will use these functions to change over time or when commanded to.
	// Also need to add some utility functions like oscillators, transitions effects, etc.
 public:
	void init();
	...
	void SetEndOfRange(EndOfRange newEndOfRange);

	void SetHue(float newHue);
	void SetHue(float newHue, float newHueSpeed);
	void SetHue(float newHue, float newHueSpeed, float newHueAcceleration);

	void SetBrightness(float newBrightness);

	void SetRange(float newRangeStart, float newRangeEnd);

	// More functions will go here but I gotta go for now :)
};
